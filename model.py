...
# 此处必须考虑样本正负比例情况，提高模型的召回率；考虑之后模型召回率显著提升
...
train_accuracy = accuracy_score(train_y, train_y_predict)
...
ks_test = max(np.abs(tpr_logit_val-fpr_logit_val))
print("the test group's ks is :", ks_test)

test_precision = precision_score(test_y,test_y_predict)
test_accuracy = accuracy_score(test_y, test_y_predict)
...
test_confusion = confusion_matrix(test_y, test_y_predict)
# 画图
